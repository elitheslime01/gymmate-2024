...
export const addStudentToQueue = async (req, res) => {
    ...
    if (
        ...
        return res.status(400).json({
            success: false,
            message: "Student ID, date, time slot (start and end times), schedule ID, and AR ID are required.",
        });
    }
...
export const checkIfStudentInQueue = async (req, res) => {
  const { studentId, date, timeSlot } = req.body;

  try {
      // Find the queue for the specified date and time slot
      ...
      if (!queue) {
          return res.status(200).json({ success: true, isInQueue: false });
      }

      // Check if the student is already in the queue
      const isInQueue = queue.students.some(
          (student) => student._studentId.toString() === studentId
      );

      return res.status(200).json({ success: true, isInQueue });
  } catch (error) {
      console.error("Error checking if student is in queue:", error.message);
      return res.status(500).json({ success: false, message: "Server error." });
    }
};